...
let
  ...
  # IAM / SSO profile `tktliam` — credentials via Granted (nix store path, not Homebrew).
  tktliamCredentialProcess = pkgs.writeShellScript "aws-credential-process-tktliam" ''
    exec ${lib.getExe' pkgs.granted "granted"} credential-process --profile tktliam "$@"
  '';
...
in
{
  ...
  home.packages = with pkgs; [
    yubikey-manager # `ykman` for aws-vault --prompt ykman (see zsh initContent above)
  ...
  # `tktliam`: Granted SSO / IAM Identity Center via `credential_process` (stable nix path).
  # Run `granted login` (brew or nix) once so Granted knows this profile; then `aws --profile tktliam sts get-caller-identity`.
  custom.aws = {
    enable = true;
    profiles = [
      {
        name = "tktliam";
        credentialProcess = "${tktliamCredentialProcess}";
      }
    ];
  };
}

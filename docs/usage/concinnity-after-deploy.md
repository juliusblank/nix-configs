...
**`tktliam`** — **`~/.aws/config`** is manually managed on concinnity (generated by a work GitHub workflow). For profiles migrated to granted, ensure `credential_process = granted credential-process --profile=tktliam` is uncommented on the base profile and `mfa_serial` is removed from role profiles (see `docs/SPEC.md` *Granted + YubiKey migration*).

**Shell commands** (all declared in **`hosts/concinnity/home.nix`**):

| Command | Tool | What it does |
|---|---|---|
| `assume <profile>` | granted | Assume role (interactive MFA) |
| `gassume <profile>` | granted | Assume role (auto YubiKey TOTP) |
| `login <profile>` | granted | Open AWS console in browser |
| `vassume <profile>` | aws-vault | Exec subshell with creds (legacy) |
| `vlogin <profile>` | aws-vault | Open console via aws-vault (legacy) |

aws-vault backend config (`AWS_VAULT_BACKEND`, `AWS_VAULT_PROMPT`, etc.) is set via `programs.zsh.sessionVariables` — no CLI flags needed.

...
| 4 | `tf-apply` guardrails | Hard block if not on `main` and working tree is dirty. Soft warn (warn + require Enter) if not on `main` but tree is clean. On `main`, runs without interruption. CI bypasses naturally (always clean, always on main). |

...
| 9 | Changelog via `git-cliff` | Done — `cliff.toml` at repo root; `just changelog` for local preview; CI `changelog.yml` regenerates `CHANGELOG.md` and commits it on every merge to `main` |
...
- Generated by CI (`.github/workflows/changelog.yml`) on every merge to `main` — not maintained by hand
- `git-cliff` is available in the devShell; run `just changelog` to regenerate locally

...
| 9 | Changelog via `git-cliff` | Done — `cliff.toml` at repo root; pre-commit hook regenerates `CHANGELOG.md` on every commit; `release.yml` workflow_dispatch creates CalVer tag + opens a release PR with re-sectioned changelog |
...
- Updated automatically by the pre-commit hook on every commit (inside the devShell) — not maintained by hand
...
This repo uses **CalVer** (`v<year>.<month>`, e.g. `v2026.04`) rather than SemVer. SemVer's
...
- **Canonical path:** trigger `.github/workflows/release.yml` via `workflow_dispatch` in the GitHub UI
  — it auto-computes the right CalVer tag, creates it, re-sections `CHANGELOG.md` under the new tag,
  and opens a PR; merge the PR via the GitHub UI to publish

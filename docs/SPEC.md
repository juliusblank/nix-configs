...
| 22 | concinnity AWS auth restoration | Three parts: (a) Rebuild `~/.aws/config` — work profiles generated by a GitHub workflow (documented in Notion), download result and place on machine; `custom.aws` currently overwrites on activation, needs split or disable. (b) ~~`assume` name conflict~~ Done — aws-vault functions renamed to `assume-vault` / `login-vault`; Granted `assume` / `login` are now primary with automatic YubiKey TOTP via `--mfa-token` + `ykman`. (c) YubiKey integration — verify `ykman` + `aws-vault --prompt ykman` still works after deploy (available as `assume-vault`). |
...
- **Concinnity:** `assume` / `login` functions source Granted's `assume` script with automatic YubiKey TOTP (`--mfa-token` + `ykman oath accounts code`); aws-vault available as `assume-vault` / `login-vault` (legacy)
- **Serenity:** `assume` shell alias via `granted.nix` module (`source assume`)
...
- Next steps: configure SSO profiles in `~/.aws/config`, then remove IAM access keys from 1Password, update `shell.nix` shellHook, and retire `aws-vault` / `assume-vault` / `login-vault`
...
| YubiKey + AWS auth | As needed | **`yubikey-manager`** (CLI **`ykman`**) on **`PATH`** via **`hosts/concinnity/home.nix`**. Granted **`assume`** / **`login`** (primary) use **`--mfa-token`** with **`ykman oath accounts code`**; aws-vault **`assume-vault`** / **`login-vault`** (legacy) use **`--prompt ykman`** |
